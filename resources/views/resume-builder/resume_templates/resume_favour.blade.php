...
<div class="resume_first">
	<div class="row">
		<div class="col-md-7 mt-5">
			<div class="row">											
				...
				@if(!empty($template_data['experience_history'][0]))
					<div class="client_detail_sec">
						 <h3 class="ml-2">WORK EXPERIENCE</h3>
					</div>
					@foreach($template_data['experience_history'] as $k=>$experience)
					<div class="row info_detail_main mt-4">																							
						<div class="col-md-12 p-0 mt-4 ml-4 pr-4">
							<p class="ml-4">@if(!empty($experience->start_date)){{date('Y',strtotime($experience->start_date))}}@endif-@if(!empty($experience->end_date)){{date('Y',strtotime($experience->end_date))}}@endif</p>
							<h4>@if(!empty($experience->position_head)){{$experience->position_head}}@endif</h4>
							<h5>@if(!empty($experience->company)){{$experience->company}}@endif<h5>													
							<ul>
				...
				@if(!empty($template_data['hobby_section'][0]))	
					<div class="client_detail_sec mt-4 ml-2">
						 <h3>HOBBIES</h3>
					</div>
					<?php 
						$hobby_names = '';
					...
					?>
					<div class="row info_detail_main mt-4">	  												   												
						<div class="col-md-12 p-0 ml-3 pr-5 pl-3">
							<div class="sect_fav">
							@foreach($template_data['hobby_section'] as $k=>$hobby)
								<h5>{{$hobby->description}}@if($k < $count_hobbies-1), @endif<h5>
							@endforeach
		...
		<div class="col-md-5 p-0">
			<div class="right_info right_cstm">
			<div class="client_info client_cv">
...
<path d="M2.97479 50C0.991597 41.3821 0 33.9024 0 27.561C0 18.4553 2.14846 11.626 6.44538 7.07317C10.7423 2.35772 17.0224 0 25.2857 0V9.7561C20.9888 9.7561 17.8487 10.8943 15.8655 13.1707C14.0476 15.4472 13.1387 18.8618 13.1387 23.4146H25.5336C25.5336 23.4146 33.4499 43.2827 25.5336 50C18.8163 55.6999 2.97479 50 2.97479 50ZM36.4412 50C34.458 41.3821 33.4664 33.9024 33.4664 27.561C33.4664 18.4553 35.6148 11.626 39.9118 7.07317C44.2087 2.35772 50.4888 0 58.7521 0V9.7561C54.4552 9.7561 51.3151 10.8943 49.3319 13.1707C47.514 15.4472 46.605 18.8618 46.605 23.4146H58.7521C58.7521 23.4146 66.8906 43.2517 59 50C52.3048 55.726 36.4412 50 36.4412 50Z" fill="white"></path>
			...
			<div class="right_info_sec right_cstm">
				<div class="client_detail_sec">
						 <h3 class="ml-2">EDUCATION</h3>
			 </div>
			 @foreach($template_data['education_history'] as $k=>$education)
				 <div class="col-md-12 p-0 mt-4 ml-2 pr-4 info_detail_main sd">
								<p class="ml-4">2015 - 2019</p>
								<h4>@if(!empty($education->school)){{$education->school}}@endif</h4>
								<h5>@if(!empty($education->achievement)){{$education->achievement}}@endif<h5>													
								<h6>@if(!empty($education->subject1)){{$education->subject1}}@endif</h6>
								<h6>@if(!empty($education->subject2)){{$education->subject2}}@endif</h6>
								<h6>@if(!empty($education->subject3)){{$education->subject3}}@endif</h6>
